...
export const RichTextEditor = ({
  ...
  return (
    <>
      <EditorTemplate>
        <EditorTemplate.Header>
          <div className="mx-auto flex max-w-[90rem] items-center gap-3">
            <IconButton label={en.navigation.backToDocuments} onClick={navigateBack}>
              <IconArrowLeft size={19} stroke={1.8} />
            </IconButton>
            <input
            ...
            />
            {!canEdit ? (
              <Button onClick={() => setSubscriptionOpen(true)} variant="secondary">
                <IconLock size={16} stroke={1.9} />
                {en.editor.readOnly}
              </Button>
            ) : (
              <IconButton label={en.documents.delete} onClick={() => setDeleteOpen(true)}>
        ...
        <EditorTemplate.Toolbar>
          <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <EditorToolbar editable={canEdit} editor={editorState.editor} />
            <SaveIndicator
      ...
      <SubscriptionModal onClose={() => setSubscriptionOpen(false)} open={subscriptionOpen} />
      <ConfirmDeleteDialog
        deleting={deleting}
